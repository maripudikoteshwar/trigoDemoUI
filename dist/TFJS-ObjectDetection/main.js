...
module.exports = "<div style=\"text-align:center\">\n  <body>\n    <div\n      style=\"background-color: #00539f;\n    height: 55px;\n    left: -20px;\n    top: -10px;\n    position: relative;\n    width: 150%;\"\n    >\n      <div\n        style=\"\n        text-align:center;\n    font-family: arial;\n    color: white;\n    font-size: 20px;\n    position: fixed;\n    margin-top: 20px;\n    font-weight: bold;\n    font-size: medium;\n    left: 45%;\n    height: 50px;\"\n      >\n        TESCO-TRIGO\n      </div>\n    </div>\n  </body>\n  <div>\n    <video hidden id=\"vid\" width=\"300\" height=\"300\"></video>\n    <canvas id=\"canvas\"></canvas>\n    <div style=\"padding:2%\">\n      <button (click)=\"goToTill()\">Go To Till</button>\n    </div>\n    <div *ngIf=\"till\">\n      <div *ngFor=\"let item of itemsAdded\">\n        <div>\n          <table style=\"padding-left: 35%\">\n            <tr>\n              <td style=\"width: 5%\">{{ item.data.item }}</td>\n              <td style=\"width: 5%\">\n                <button (click)=\"scanItem(item)\">scan</button>\n              </td>\n            </tr>\n          </table>\n        </div>\n      </div>\n      <!-- <div *ngFor=\"let item of itemsAdded\">\n          <div>\n            <ul>\n              <li style=\"flex-direction: row; display: flex\" >\n                <div>\n                    {{ item.data.item }}\n                </div>\n                <div>\n                    <button (click)=\"scanItem(item)\">scan</button>\n                </div>\n              </li>\n            </ul>\n          </div>\n        </div> -->\n      <button (click)=\"payBill()\">Pay</button>\n    </div>\n  </div>\n</div>\n"
...
var AppComponent = /** @class */ (function () {
    ...
    function AppComponent() {
        ...
        this.objectDetectionThreshold = 10;
        this.personThreshold = 100;
        this.objectsDetected = new Map();
        this.objectsSent = new Set();
        this.shopping = false;
        this.personInFrames = [];
        this.storeId = 9763;
        this.itemsAdded = [];
        this.till = false;
        this.itemsScanned = [];
        this.handleTrigoClientMessages = function (message) {
            ...
            if (messageObj.type == "personidentified") {
                var dataToSend = {
                    ...
                    ackId: Object(uuid__WEBPACK_IMPORTED_MODULE_2__["v4"])(),
                    time: new Date(),
        ...
        this.sendMessage = function (message) {
            try {
                ...
                if (message == "person") {
                    _this.personId = Object(uuid__WEBPACK_IMPORTED_MODULE_2__["v4"])();
                    _this.sessionId = Object(uuid__WEBPACK_IMPORTED_MODULE_2__["v4"])();
                    dataToSend = __assign({}, dataToSend, { type: "sessionstarted", data: { personId: _this.personId, sessionId: _this.sessionId } });
                    dataToSend = JSON.stringify(dataToSend);
                    _this.wsClient.send(dataToSend);
                    _this.shopping = true;
                }
                else if (message == "shopping completed") {
                    dataToSend = __assign({}, dataToSend, { type: "sessionended", data: {
                            personId: _this.personId,
                            sessionId: _this.sessionId
                        } });
                    dataToSend = JSON.stringify(dataToSend);
                ...
                else if (_this.shopping) {
                    dataToSend = __assign({}, dataToSend, { type: "itemsdetected", data: {
                            personId: _this.personId,
                            sessionId: _this.sessionId,
                            item: message
                        } });
                    _this.itemsAdded.push(dataToSend);
                    dataToSend = JSON.stringify(dataToSend);
                    _this.wsClient.send(dataToSend);
                }
                console.log(dataToSend);
        ...
        this.shouldShoppingEnd = function () {
            if (_this.personInFrames.length < _this.personThreshold)
                return false;
            for (var i = 0; i < _this.personInFrames.length; i++) {
                if (_this.personInFrames[i]) {
                    return false;
                }
            }
            return true;
        };
        this.handlePredictions = function (predictions) {
            var personInFrame = false;
            predictions.forEach(function (prediction) {
                if (!_this.objectsDetected.has(prediction.class)) {
                    _this.objectsDetected.set(prediction.class, 1);
                }
                else {
                    _this.objectsDetected.set(prediction.class, _this.objectsDetected.get(prediction.class) + 1);
                }
                if (prediction.class == "person") {
                    personInFrame = true;
                }
            });
            _this.personInFrames.push(personInFrame);
            if (_this.personInFrames.length > _this.personThreshold) {
                _this.personInFrames.shift();
            }
            _this.objectsDetected.forEach(function (value, key) {
                if (value > _this.objectDetectionThreshold && !_this.objectsSent.has(key)) {
                    _this.sendMessage(key);
                    _this.objectsSent.add(key);
                }
            });
            if (!personInFrame && _this.shopping && _this.shouldShoppingEnd()) {
                _this.shopping = false;
                _this.objectsDetected.clear();
                _this.objectsSent.clear();
                _this.sendMessage("shopping completed");
            }
        };
        ...
        this.renderPredictions = function (predictions) {
            ...
            _this.handlePredictions(predictions);
            predictions.forEach(function (prediction) {
    ...
    AppComponent.prototype.goToTill = function () {
        this.till = true;
    ...
    AppComponent.prototype.scanItem = function (item) {
        this.itemsScanned.push(item);
        console.log(item.data.item + " Scanned");
    };
    AppComponent.prototype.payBill = function () {
        console.log("Items Scanned at Till,");
        this.itemsScanned.forEach(function (item) { return console.log(item.data.item); });
    };
...
module.exports = __webpack_require__(/*! /Users/maripudi/Desktop/objectDetection/TRIGO_DemoUI/src/main.ts */"./src/main.ts");
